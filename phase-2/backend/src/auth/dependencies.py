...
async def get_current_user(
    ...
    """
    FastAPI dependency to extract authenticated user from Better Auth JWT token.

    Validates JWT token generated by Better Auth server and fetches
    ...
    try:
        ...
        # Try multiple claim names for compatibility
        user_id_str = (
        ...
        if not user_id_str:
            raise HTTPException(
                status_code=401,
                detail="Invalid token - missing userId/user_id/sub claim",
            )
        ...
        # Re-raise JWT validation errors from verify_token
        raise

    # Fetch user from database (Better Auth uses 'user' table)
    user = session.get(User, user_id)

    if not user:
        raise HTTPException(
            status_code=401,
            detail="User not found - token may be for deleted account",
        )
